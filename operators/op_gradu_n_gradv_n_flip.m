% OP_GRADU_N_GRADV_N_FLIP: assemble the matrix A = [a(i,j)], a(i,j) = (epsilon (grad u n)_j, (grad v n)_i), with n the normal vector (flipped).
%
%   mat = op_gradu_n_gradv_n_flip (spu, spv, msh, epsilon);
%   [rows, cols, values] = op_gradu_n_gradv_n_flip (spu, spv, msh, epsilon);
%
% INPUT:
%
...
function varargout = op_gradu_n_gradv_n_flip (spu, spv, msh, coeff)
  ...
  for iel = 1:msh.nel
    if (all (msh.jacdet(:,iel)))
      ...
      [rows_loc, cols_loc] = ndgrid (connv(:,iel), spu.connectivity(:,iel));
