% OP_LAPLACEU_LAPLACEV_HIER: assemble the stiffness matrix A = [a(i,j)], a(i,j) = (epsilon laplace u_j, laplace v_i), 
%  for hierarchical splines, exploiting the multilevel structure.
%
%   mat = op_laplaceu_laplacev_hier (hspu, hspv, hmsh, [epsilon]);
%   [rows, cols, values] = op_laplaceu_laplacev_hier (hspu, hspv, hmsh, [epsilon]);
%
% INPUT:
%
...
function varargout = op_laplaceu_laplacev_hier (hspu, hspv, hmsh, coeff)
  ...
  for ilev = 1:hmsh.nlevels
    ...
    if (hmsh.nel_per_level(ilev) > 0)
      ...
      for idim = 1:hmsh.rdim
        ...
        spu_lev = sp_evaluate_element_list (hspu.space_of_level(ilev), hmsh.msh_lev{ilev}, 'value', false, 'laplacian', true);
        spv_lev = sp_evaluate_element_list (hspv.space_of_level(ilev), hmsh.msh_lev{ilev}, 'value', false, 'laplacian', true);
