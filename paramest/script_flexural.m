%% Parameter identification of the flexural parameter
...
prb.tp = @(x, y) eval_uv(prb.topo, x, y, prb.grsz) * prb.km; % rock-equivalent topography in ul
...
opt.init_dsn = 1e3 * prb.em(0, 0) * prb.th(0, 0)^3 / (12 * (1 - prb.nu(0, 0)^2)) * ones(opt.spc.ndof, 1);
opt.final_dsn = descent_method_positive(@(dsn) problem_flexural(1e-3 * dsn, prb, mdl, sim, opt), opt.init_dsn, options);

[obj, dsc, nrm, prb, mdl, sim] = problem_flexural(1e-3 * opt.final_dsn, prb, mdl, sim, opt);
...
vis_contour_map(lon, lat, 1e-1 * nthroot(12 * (1 - prb.nu(0, 0)^2) * val / prb.em(0, 0), 3) * prb.ul, 'fname', 'Estimated thickness from observed data', 'cname', 'Thickness (km)');
