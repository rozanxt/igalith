%% Parameter identification of the reference rock density
...
opt.init_dsn = prb.wr(0, 0) / (prb.gr / prb.km^3) * ones(opt.spc.ndof, 1);
opt.final_dsn = descent_method(@(dsn) problem_density(dsn * (prb.gr / prb.km^3), prb, mdl, sim, opt), opt.init_dsn, options);

[obj, dsc, nrm, prb, mdl, sim] = problem_density(opt.final_dsn * (prb.gr / prb.km^3), prb, mdl, sim, opt);
...
vis_contour_map(lon, lat, val, 'fname', 'Estimated density from observed data', 'cname', 'Density (g/cm^3)');
