% First-order iterative descent method for parameter identification
% with restriction to positive design variables
function prev_dsn = descent_method_positive(problem, init_dsn, options)
    lower_bound = eps;
    ...
    while ~terminate
        ...
        options.output_fcn(prev_dsn, prev_obj, desc_dir, grad_nrm, step_size);
        ...
        next_dsn(next_dsn < lower_bound) = lower_bound;
        ...
        if next_obj < prev_obj - step_size * options.sigma * grad_nrm^2
            while true
                ...
                temp_dsn(temp_dsn < lower_bound) = lower_bound;
